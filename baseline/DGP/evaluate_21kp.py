...
def gen_tree(args,nodes_name):
    graph_edges = json.load(open(args.graph_path, 'r'))
...
class tree():
    def __init__(self,args,nodes_name):
        self.p2c, self.c2p, self.d2n, self.nodes, self.start_up = gen_tree(args,nodes_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cnn',default='materials/resnet50-base.pth',help='FSL/dgp/1/epoch-5.pth')
    parser.add_argument('--pred',default='gcn_dense_trans/epoch-3000.pred')

    parser.add_argument('--test-set',default='hop2')
    ...
    parser.add_argument('--split', default='zsl_test',type=str)
    parser.add_argument('--graph_path', default='21kp_test',type=str)

    args = parser.parse_args()

    set_gpu(args.gpu)

    test_sets = json.load(open('process_results/splits_for_hops.json', 'r'))
    # train_wnids = test_sets['train']
    test_wnids = test_sets[args.test_set]

    nodes_name=test_wnids
    tree=tree(args,nodes_name)

    test_index = torch.from_numpy(np.arange(0,len(nodes_name))).cuda()
    ...
    pred_vectors = pick_vectors(pred_dic,  test_wnids, is_tensor=True).cuda()
